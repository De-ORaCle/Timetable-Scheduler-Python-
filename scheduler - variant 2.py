...
special_conflict_courses = {
    "botany": ["CHE 127", "PHY 101", "ZOO 115", "MIC 121", "CHE 176", "CHE 259", "BIO 212"],
    "computer_science": ["MAT 353"],
    "zoology": ["BOT 415", "BIO 413", "BIO 411", "BIO 414", "BIO 212"],
    "mathematics": ["STA 121", "PHY 118", "STA 131", "CHE 176"]
}
...
# Define the function to check if a course is already assigned to a day and department
def is_course_assigned(day, department, course):
    for time in schedule[day]:
        for assigned_course in schedule[day][time]:
            if assigned_course.startswith(department) and assigned_course != course:
                return True
    return False
...
for level, courses_by_department in courses.items():
    for department, department_courses in courses_by_department.items():
        assigned_courses = set()  # Keep track of assigned courses for the department
        for course in department_courses:
            assigned = False
            while not assigned:
                day = random.choice(list(schedule.keys()))
                start_time = random.randint(8, 14)
                end_time = start_time + 2  # Each course gets 3 hours per week
                if not has_course_conflict(day, start_time, end_time, department) and not is_course_assigned(day, department, course):
                    assign_course(day, start_time, end_time, course)
                    assigned_courses.add(course)
                    assigned = True

# Assign the conflict courses
for department, conflict_courses in special_conflict_courses.items():
    for conflict_course in conflict_courses:
        ...
        while not assigned:
            ...
            start_time = random.randint(8, 14)
            end_time = start_time + 2  # Each course gets 3 hours per week
            if not has_course_conflict(day, start_time, end_time, department) and not is_course_assigned(day, department, conflict_course):
