...
# Define the function to check for course conflicts (1-hour intervals)
...
# Define the function to assign a course to a time slot (1-hour intervals)
...
# Adjusting the time range to 1-hour slots from 8 am to 6 pm
schedule = {}
for day in ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]:
    schedule[day] = {time: [] for time in range(8, 19)}  # Updated the range to include 18 (6 pm)


# Assign the courses for each level and department (1-hour intervals)
    ...
    for department, department_courses in courses_by_department.items():
        for course in department_courses:
            ...
            while not assigned:
                ...
                start_time = random.randint(0, 8) + 8  # Random start time from 8 am to 5 pm in 1-hour slots
                end_time = start_time + 2  # Each course gets 2 hour per day
...
# Assign the conflict courses based on levels (1-hour intervals)
...
table = []
time_slots = list(range(8, 17, 1))  # Time slots from 8 am to 5 pm in 1-hour intervals
